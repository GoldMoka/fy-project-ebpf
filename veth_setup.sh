...
#  Every firewall interface (fw*) stays in root netns where main.py runs.
#  Every attacker interface (atk*) lives in xdp_attacker netns.
#  Coordinator backbone links stay in root netns.
#
...
setup_ring_8() {
    ...
    for i in $(seq 0 7); do
        ...
        write_peers "$SCRIPT_DIR/peers_ring${i}.json" \
            "{\"role\":\"leaf\",\"topology_hint\":\"ring\",\"next\":\"${ips[$nxt]}\",\"ring_size\":8}"
    done
    ok "ring-8 ready — fw-ring-{0..7} / 10.20.{0..7}.1"
}
teardown_ring_8() {
    for i in $(seq 0 7); do del_if "fw-ring-${i}"; done
}
...
EOF
    ...
    ring-8)
        for i in $(seq 0 7); do
            echo "  # T$((i+1)) — ring node $i"
            echo "  sudo python3 main.py --iface fw-ring-${i} --port $((7000+i)) --peer-port $((7000+((i+1)%8))) \\"
            echo "       --topology ring --peers-file peers_ring${i}.json --xdp-mode native --hmac-key \$KEY"
        done
        echo ""
        echo "  # Benchmark ring-0 (B5 shows 8-hop propagation latency)"
        echo "  sudo bash benchmark.sh --iface fw-ring-0 --target 10.20.0.1 --runs 5 --veth --hmac-key \$KEY"
        ;;
    hierarchical)
...
do_benchmark() {
    ...
    ring-8)
        echo "  # Benchmark each node — B5 gossip latency scales with hop count"
        for i in $(seq 0 7); do
            echo "  sudo bash benchmark.sh --iface fw-ring-${i} --target 10.20.${i}.1 \\"
...
_node_cmds() {
    ...
    ring-8)
        for i in $(seq 0 7); do
            echo "fw-ring-${i} ${PY} --iface fw-ring-${i} --port $((7000+i)) --peer-port $((7000+(i+1)%8)) --topology ring --peers-file ${SCRIPT_DIR_local}/peers_ring${i}.json --xdp-mode native --hmac-key ${KEY}"
        done
        ;;
    hierarchical)
